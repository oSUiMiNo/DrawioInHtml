...
```html
<script type="application/xml" data-drawio-id="my-diagram">
<mxGraphModel>
  <root>
    <mxCell id="0"/>
...
- `data-drawio-id` の値（上の例では `"my-diagram"`）は、同じ HTML ファイル内で重複しないユニークな名前にする
- 中身は `<mxGraphModel>...</mxGraphModel>` の Drawio XML を貼り付ける
- まずは適当な名前で空の `<script>` タグを置き、✏️ から Drawio エディタで描き始めるのが楽です

> ブラウザでも図を表示したい場合や、旧形式 (`type="application/drawio+xml"`) との互換情報は [開発者向けドキュメント](./README_DEV.html) を参照してください。

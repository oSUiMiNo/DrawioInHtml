...
このマーカーは HTML5 標準の「インラインデータブロック」用法そのものなので、ブラウザは中身を無視し、副作用なく共存します。**ブラウザでも図を表示したい場合**は、同じ `id` を読み出して描画する自前マウントを追加してください — 具体例は [開発者向けドキュメント](./README.dev.html) を参照。
